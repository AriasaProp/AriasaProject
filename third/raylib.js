...
class RaylibJs {
  ...
  DrawCircleV(center_ptr, radius, color_ptr) {
    const buffer = this.exports.memory.buffer;
    const [x,
      y] = new Float32Array(buffer, center_ptr, 2);
    const [r,
      g,
      b,
      a] = new Uint8Array(buffer, color_ptr, 4);
  ...
  CheckCollisionPointRec(point_ptr, rec_ptr) {
    const buffer = this.exports.memory.buffer;
    const [x,
      y] = new Float32Array(buffer, point_ptr, 2);
    const [rx,
      ry,
      rw,
      rh] = new Float32Array(buffer, rec_ptr, 4);
  ...
  Fade(result_ptr, color_ptr, alpha) {
    ...
    const [r,
      g,
      b,
      _] = new Uint8Array(buffer, color_ptr, 4);
  ...
  DrawRectangleRec(rec_ptr, color_ptr) {
    ...
    const [x,
      y,
      w,
      h] = new Float32Array(buffer, rec_ptr, 4);
    const color = getColorFromMemory(buffer, color_ptr);
    this.ctx.fillStyle = color;
  ...
  DrawTexture(texture_ptr, posX, posY, color_ptr) {
    ...
    const [id,
      ...
      height,
      mipmaps,
      format] = new Uint32Array(buffer, texture_ptr, 5);
  ...
  DrawTextEx(font, text_ptr, position_ptr, fontSize, spacing, tint_ptr) {
    ...
    const text = cstr_by_ptr(buffer, text_ptr);
    const [posX,
      posY] = new Float32Array(buffer, position_ptr, 2);
